    ...
    void Update () {
        if(time+AttackInterval<Time.time)
        {
            ...
            if(rnd<0.2)
            {
                ...
                for(int i=0;i<10; i++)
                {
                    float rndy=Random.Range(-2.5f,1.5f);
                    float rndx = Random.Range(-4.1f, -4f);
                    float direction = Mathf.Sign(Random.Range(-1f, 1f));
                    if(direction>0)//face right
                    {
                        Instantiate(bulletEnemy, new Vector3(direction*rndx, rndy, 0), Quaternion.identity);
                    }
                    else
                    {
                        Instantiate(bulletReverseEnemy, new Vector3(direction*rndx, rndy, 0), Quaternion.identity);
                    }
                }
            }
            else if(rnd>=0.2&&rnd<0.4f)
            {
                ...
                transform.rotation = Quaternion.Slerp(transform.rotation, Quaternion.Euler(0, 30, 0),0.01f*Time.deltaTime);
                for (int i = 0; i < 5; i++)
                {
                    float rndy = Random.Range(-2.5f, 1.5f);
                    float rndx = Random.Range(-4.1f, -4f);
            ...
            else if (rnd >= 0.4 && rnd < 0.6f)
            {
                ...
                {
                    float rndy = Random.Range(-2.5f, 1.5f);
                    float rndx = Random.Range(4f, 4.1f);
            ...
            else if (rnd >= 0.6 && rnd < 0.8f)
            {
                ...
                transform.rotation = Quaternion.Slerp(transform.rotation, Quaternion.Euler(30, 60, 0), 0.01f * Time.deltaTime);
                for (int i = 0; i < 20; i++)
                {
                    float rndy = Random.Range(-2.5f, 1.5f);
        ...
        {
            player.GetComponent<BasicParams>().HorizonDefeated = true;
